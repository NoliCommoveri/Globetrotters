...
const WEEK_SIZES = { 1: 12, 2: 86, 3: 30, 4: 30 };
...
// 41-66 prompts above baseline and 10-40 on-theme; against the 114 drawable
...
test('158 task templates, distributed 12 / 86 / 30 / 30', () => {
...
test('114 prompts are drawable — weeks 2 and 3, minus the two pins', () => {
  const drawable = rows(
    "SELECT id FROM task_templates WHERE week_theme IN (2, 3) AND tier != 'fixed'"
  ).length;
  assert.equal(drawable, 114);
});
