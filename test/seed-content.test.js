...
const WEEK_SIZES = { 1: 12, 2: 70, 3: 25, 4: 30 };
...
// 41-66 prompts above baseline and 10-40 on-theme; against the 93 drawable
...
test('137 task templates, distributed 12 / 70 / 25 / 30', () => {
...
test('93 prompts are drawable — weeks 2 and 3, minus the two pins', () => {
  const drawable = rows(
    "SELECT id FROM task_templates WHERE week_theme IN (2, 3) AND tier != 'fixed'"
  ).length;
  assert.equal(drawable, 93);
});
